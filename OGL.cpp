...
GLuint vbo_Cube_Color = 0;  // each shapes increases the readability and clean code it is good practise

GLuint mvp_MatrixUniform = 0;  // mvp - model view  projection matrix
...
int initialise(void)
{
    ...
    mvp_MatrixUniform = glGetUniformLocation(shaderProgramObject, "uMVPMatrix");
...
void display(void)
{
    ...
    //-------------------------Cube---------------------------------------
        //trasformations
        mat4 modelViewMatrix = mat4::identity(); // analogues to glloadidentity() in display 
        ...
        glUniformMatrix4fv(mvp_MatrixUniform, 1, GL_FALSE, modelViewProjectionMatrix);
